...
def set_up_hyperparams(s=None):
    ...
    parse_args_and_update_hparams(H, parser, s=s) #更新
    setup_save_dirs(H) #保存路径
    logprint = logger(H.logdir) #打印路径
    for i, k in enumerate(sorted(H)): #打印
        logprint(type='hparam', key=k, value=H[k])
    np.random.seed(H.seed)  #np.random随机
    torch.manual_seed(H.seed)  #torch 随机
    torch.cuda.manual_seed(H.seed)  #torch cuda 随机
